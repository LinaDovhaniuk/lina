...
void terminal_printAllViolation(post_t * self){
	...
	for (int i = 0; i < post_getNumOfCars(self); i++){
		...
		if (post_getMachineViolationEnum(self, i) == stopping_rule || post_getMachineViolationEnum(self, i) == prohibited_signal ||
			post_getMachineViolationEnum(self, i) == exit_at_crossroads || post_getMachineViolationEnum(self, i) == speeding){
